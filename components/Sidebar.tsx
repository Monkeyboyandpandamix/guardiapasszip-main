...
const Sidebar: React.FC<SidebarProps> = ({ activeSection, setActiveSection, isSecure, activeColor = 'emerald' }) => {
      ...
      `}>
        <div className="p-6 lg:p-8">
          <div className="mb-10 flex items-center gap-3">
             ...
             <span className="font-black text-xl text-white tracking-tighter uppercase">Guardia<span className={`text-${activeColor}-500`}>Pass</span></span>
          </div>
          <nav className="space-y-2">
            {items.map((item) => (
              ...
              >
                <item.icon className={`w-5 h-5 ${activeSection === item.id ? `text-${activeColor}-400` : 'text-slate-600'}`} />
        ...
        <div className="mt-auto p-6">
          <div className={`p-5 rounded-3xl border transition-all duration-500 ${isSecure ? `border-emerald-500/20 bg-emerald-950/20` : 'border-slate-800 bg-slate-900/50'}`}>
            <div className="flex items-center justify-between mb-3">
              <p className={`text-[9px] font-black uppercase tracking-widest ${isSecure ? `text-emerald-400` : 'text-slate-600'}`}>Bridge Status</p>
              {isSecure ? <ShieldCheck className={`w-3 h-3 text-emerald-400`} /> : <ShieldX className="w-3 h-3 text-slate-600" />}
            </div>
            <div className="flex items-center gap-3">
              <div className="relative">
                <div className={`w-2 h-2 rounded-full ${isSecure ? `bg-emerald-500` : 'bg-slate-700'}`} />
                {isSecure && <div className={`absolute inset-0 w-2 h-2 rounded-full bg-emerald-500 animate-ping`} />}
              </div>
